...
import {
  ...
  addSourcesBatch as addSourcesBatchOnPage,
  type AddSourceInput,
  type AddSourceResult,
  type BatchAddSourceResult,
...
export class BrowserSession {
  ...
  /**
   * Ingest multiple sources in batch to the active notebook.
   */
  async addSourcesBatch(inputs: AddSourceInput[]): Promise<BatchAddSourceResult> {
    if (!this.initialized || !this.page || this.isPageClosedSafe()) {
      await this.init();
    }
    return await addSourcesBatchOnPage(this.page!, inputs);
  }

  /**
   * List existing source titles currently indexed in the notebook sidebar.
   */
  async listSources(): Promise<string[]> {
    if (!this.initialized || !this.page || this.isPageClosedSafe()) {
      await this.init();
    }
    return await listSourcesOnPage(this.page!);
  }
