...
import { extractLocalFile } from "../utils/file-extractor.js";
...
export interface AddSourceInput {
  type: SourceType;
  /** URL when `type === "url" | "youtube"`, raw text when `type === "text"`, file path when `type === "file"`. */
  content: string;
  /** Optional title shown in the source list. NotebookLM uses a default if omitted. */
  title?: string;
  /** Optional explicit file path when type === "file". If omitted, `content` is treated as the path. */
  path?: string;
}
...
export async function addSource(page: Page, input: AddSourceInput): Promise<AddSourceResult> {
  ...
  let effectiveInput: AddSourceInput = { ...input };
  if (input.type === "file") {
    const filePath = input.path || input.content;
    const extracted = await extractLocalFile(filePath, input.title);
    effectiveInput = {
      type: "text",
  ...
  try {
    ...
    await pickSourceType(page, effectiveInput.type);

    // 3. Fill the content + optional title.
    await fillSourceContent(page, effectiveInput);
...
async function pickSourceType(page: Page, type: SourceType): Promise<void> {
  let candidates: readonly string[] = Selectors.sources.sourceTypeText;
  if (type === "url") {
    candidates = Selectors.sources.sourceTypeUrl;
  } else if (type === "youtube") {
    candidates = [...Selectors.sources.sourceTypeYoutube, ...Selectors.sources.sourceTypeUrl];
...
/**
 * List all source titles currently present in the notebook sidebar.
 */
export async function listSources(page: Page): Promise<string[]> {
  try {
    const elements = page.locator(Selectors.sources.sourceContainer);
...
/**
 * Ingest multiple sources in batch into the active notebook.
 */
export async function addSourcesBatch(page: Page, inputs: AddSourceInput[]): Promise<BatchAddSourceResult> {
  const results: Array<{ title?: string; type: SourceType; success: boolean; error?: string }> = [];
  let succeeded = 0;
  let failed = 0;

  for (const item of inputs) {
    const displayTitle = item.title || item.path || item.content.slice(0, 40);
    try {
      const res = await addSource(page, item);
      if (res.success) {
        succeeded++;
        results.push({ title: displayTitle, type: item.type, success: true });
      } else {
        failed++;
        results.push({ title: displayTitle, type: item.type, success: false, error: res.message });
      }
    } catch (err) {
      failed++;
      const msg = err instanceof Error ? err.message : String(err);
      results.push({ title: displayTitle, type: item.type, success: false, error: msg });
    }
