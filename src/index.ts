...
## Source ingestion (multi-source & multi-format)

- \`add_source\`: Ingest single source. Supports:
  • \`file\`: Local PDF, markdown, text, CSV files (e.g. \`path: "C:/docs/paper.pdf"\`)
  • \`url\`: Website URL crawled and indexed
  • \`youtube\`: YouTube video link (auto-indexes transcript)
  • \`text\`: Raw copied text
- \`add_sources_batch\`: Ingest an array of multiple sources in a single call.
- \`list_sources\`: Enumerate all sources currently indexed in the notebook.

## Deep Research & Synthesis

- \`find_contradictions\`: Cross-references sources to discover conflicting data, differing numbers, and divergent perspectives.
- \`identify_knowledge_gaps\`: Identifies blind spots and missing aspects against a research goal.
- \`export_research_dossier\`: Exports full session Q&A turns, citations, and summaries to a Markdown file (.md).
- \`export_citations\`: Formats sources into BibTeX and APA 7th edition.
...
- Free Google accounts: 50 NotebookLM queries/day. \`re_auth\` rotates accounts.
      ...
      try {
        ...
        switch (name) {
          ...
          case "add_sources_batch":
            result = await this.toolHandlers.handleAddSourcesBatch(args as any);
            break;

          case "list_sources":
            result = await this.toolHandlers.handleListSources(args as any);
            break;

          case "find_contradictions":
            result = await this.toolHandlers.handleFindContradictions(args as any);
            break;

          case "identify_knowledge_gaps":
            result = await this.toolHandlers.handleIdentifyKnowledgeGaps(args as any);
            break;

          case "export_research_dossier":
            result = await this.toolHandlers.handleExportResearchDossier(args as any);
            break;

          case "export_citations":
            result = await this.toolHandlers.handleExportCitations(args as any);
            break;
